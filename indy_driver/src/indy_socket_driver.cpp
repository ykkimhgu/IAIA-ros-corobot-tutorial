...
#include <indy_driver/grip_state.h>
#include <indy_driver/grip_command.h>
...
void readGripCommand(const indy_driver::grip_command::ConstPtr& _msg){
...
void readGripState(const indy_driver::grip_state::ConstPtr& _msg){
...
{
	ros::init(argc, argv, "indy_socket_driver");
	ros::NodeHandle n;
	...
	ros::Publisher  pub_GripState 	= n.advertise<indy_driver::grip_state>("grip_state", 10);
	ros::Subscriber sub_GripCommand = n.subscribe<indy_driver::grip_command>("grip_command", 10, readGripCommand);
	ros::Subscriber sub_GripState 	= n.subscribe<indy_driver::grip_state>("grip_state", 10, readGripState);
	
	sensor_msgs::JointState joint_state;
	indy_driver::grip_state	msg_GripState;
	...
	while (ros::ok()) {
		...
		if (indySocket.isWorking())	{
			...
			if (gripChange == 1){
				...
				else if (DO_Val0 == 1 && DO_Val1 == 0)	msg_GripState.on = 1;
